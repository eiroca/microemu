 ...
 * <code>javax.microedition.io.HttpConnection</code>. The values in this interface represent the
 * values defined in the IrOBEX specification. The values in
 * <code>javax.microedition.io.HttpConnection</code> represent values defined in the HTTP
 * specification.
 * <P>
 * <code>OBEX_DATABASE_FULL</code> and <code>OBEX_DATABASE_LOCKED</code> require further description
 * since they are not defined in HTTP. The server will send an <code>OBEX_DATABASE_FULL</code>
 * message when the client requests that something be placed into a database but the database is
 * full (cannot take more data). <code>OBEX_DATABASE_LOCKED</code> will be returned when the client
 * wishes to access a database, database table, or database record that has been locked.
 * 
