...
/**
 * The <code>DeviceClass</code> class represents the class of device (CoD) record as defined by the
 * Bluetooth specification. This record is defined in the Bluetooth Assigned Numbers document and
 * contains information on the type of the device and the type of services available on the device.
 * <P>
 * The Bluetooth Assigned Numbers document
 * (<A HREF="http://www.bluetooth.org/assigned-numbers/baseband.htm">
 * http://www.bluetooth.org/assigned-numbers/baseband.htm</A>) defines the service class, major
 * device class, and minor device class. The table below provides some examples of possible return
...
public class DeviceClass {
  ...
  /**
   * Creates a <code>DeviceClass</code> from the class of device record provided.
   * <code>record</code> must follow the format of the class of device record in the Bluetooth
   * specification.
   *
  ...
  /**
   * Retrieves the major service classes. A device may have multiple major service classes. When
   * this occurs, the major service classes are bitwise OR'ed together.
   *
