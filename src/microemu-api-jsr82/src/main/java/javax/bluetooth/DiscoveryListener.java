...
/**
 * The <code>DiscoveryListener</code> interface allows an application to receive device discovery
 * and service discovery events. This interface provides four methods, two for discovering devices
 * and two for discovering services.
 * 
 * @version 1.0 February 11, 2002
 * 
 * @since 1.1 The JSR 82 specification does not require that implementations create individual
 *        threads for event delivery. Thus, if a DiscoveryListener method does not return or the
 *        return is delayed, the system may be blocked. So the following note is given for
...
public interface DiscoveryListener {
  ...
  /**
   * Indicates device discovery has been canceled by the application and did not complete. Used with
   * the {@link #inquiryCompleted(int)} method.
   * <p>
   ...
   * @see DiscoveryAgent#searchServices(int[], javax.bluetooth.UUID[], javax.bluetooth.RemoteDevice,
   *      javax.bluetooth.DiscoveryListener)
   */
  public static final int SERVICE_SEARCH_COMPLETED = 0x01;

  /**
   * Indicates the service search has been canceled by the application and did not complete. Used
   * with the {@link #serviceSearchCompleted(int, int)} method.
   * <p>
  ...
  /**
   * Indicates the service search has completed with no service records found on the device. Used
   * with the {@link #serviceSearchCompleted(int, int)} method.
   * <p>
   ...
   * {@link DiscoveryAgent#searchServices(int[], javax.bluetooth.UUID[], javax.bluetooth.RemoteDevice, javax.bluetooth.DiscoveryListener)
   * DiscoveryAgent.searchServices()} could not be reached. Used with the
   * {@link #serviceSearchCompleted(int, int)} method.
   * <P>
   ...
   * values in the {@code respCode} argument include {@link #SERVICE_SEARCH_COMPLETED},
   * {@link #SERVICE_SEARCH_TERMINATED}, {@link #SERVICE_SEARCH_ERROR},
   * {@link #SERVICE_SEARCH_NO_RECORDS} and {@link #SERVICE_SEARCH_DEVICE_NOT_REACHABLE}. The
   * following table describes when each {@code respCode} will be used:
   * <table>
   * <tr>
   * <th>respCode</th>
   * <th>Reason</th>
   * </tr>
   * <tr>
   * <td>{@link #SERVICE_SEARCH_COMPLETED}</td>
   * <td>if the service search completed normally</td>
   * </tr>
   * <tr>
   * <td>{@link #SERVICE_SEARCH_TERMINATED}</td>
   * <td>if the service search request was cancelled by a call to
   * {@link DiscoveryAgent#cancelServiceSearch(int)}</td>
   * </tr>
   * <tr>
   * <td>{@link #SERVICE_SEARCH_ERROR}</td>
   * <td>if an error occurred while processing the request</td>
   * </tr>
   * <tr>
   * <td>{@link #SERVICE_SEARCH_NO_RECORDS}</td>
   * <td>if no records were found during the service search</td>
   * </tr>
   * <tr>
   * <td>{@link #SERVICE_SEARCH_DEVICE_NOT_REACHABLE}</td>
   * <td>if the device specified in the search request could not be reached or the local device
   * could not establish a connection to the remote device</td>
   * </tr>
   * </table>
   * 
  ...
  /**
   * Called when an inquiry is completed. The {@code discType} will be {@link #INQUIRY_COMPLETED} if
   * the inquiry ended normally or {@link #INQUIRY_TERMINATED} if the inquiry was canceled by a call
   * to {@link DiscoveryAgent#cancelInquiry(DiscoveryListener)}. The {@code discType} will be
   * {@link #INQUIRY_ERROR} if an error occurred while processing the inquiry causing the inquiry to
   * end abnormally.
   * 
   * @param discType the type of request that was completed; either {@link #INQUIRY_COMPLETED},
   *        {@link #INQUIRY_TERMINATED}, or {@link #INQUIRY_ERROR}
