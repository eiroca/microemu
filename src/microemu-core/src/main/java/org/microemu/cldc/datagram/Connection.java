/**
 * MicroEmulator Copyright (C) 2006 Bartek Teodorczyk <barteo@barteo.net> Copyright (C) 2007 Ludovic
 * Dewailly <ludovic.dewailly@dreameffect.org>
 *
...
public class Connection implements DatagramConnection, UDPDatagramConnection, ConnectionImplementation {
  ...
  @Override
  public void send(final Datagram dgram) throws IOException {
    socket.send(((DatagramImpl)dgram).getDatagramPacket());
  }

  @Override
  public void receive(final Datagram dgram) throws IOException {
    socket.receive(((DatagramImpl)dgram).getDatagramPacket());
  }

  @Override
  public Datagram newDatagram(final int size) throws IOException {
    return newDatagram(size, address);
  }

  @Override
  public Datagram newDatagram(final int size, final String addr) throws IOException {
    if (!addr.startsWith(Connection.PROTOCOL)) { throw new IllegalArgumentException("Invalid Protocol " + addr); }
    final Datagram datagram = new DatagramImpl(size);
    datagram.setAddress(addr);
    return datagram;
  }

  @Override
  public Datagram newDatagram(final byte[] buf, final int size) throws IOException {
    return newDatagram(buf, size, address);
  }

  @Override
  public Datagram newDatagram(final byte[] buf, final int size, final String addr) throws IOException {
    if (!addr.startsWith(Connection.PROTOCOL)) { throw new IllegalArgumentException("Invalid Protocol " + addr); }
    final Datagram datagram = new DatagramImpl(buf, size);
    datagram.setAddress(addr);
    return datagram;
  }
  ...
  public javax.microedition.io.Connection openConnection(final String name, final int mode, final boolean timeouts) throws IOException {
    ...
    if (!name.startsWith(Connection.PROTOCOL)) { throw new IOException("Invalid Protocol " + name); }
    // TODO currently we ignore the mode
    address = name.substring(Connection.PROTOCOL.length());
    int port = -1;
    final int index = address.indexOf(':');
    if (index == -1) { throw new IllegalArgumentException("Port missing"); }
    final String portToParse = address.substring(index + 1);
    ...
    if (index == 0) {
      ...
      // client mode
      if (port == -1) { throw new IllegalArgumentException("Port missing"); }
