...
public class Connection extends org.microemu.cldc.http.Connection implements HttpsConnection {
  ...
  public SecurityInfo getSecurityInfo() throws IOException {
    if (securityInfo == null) {
      ...
      final HttpsURLConnection https = (HttpsURLConnection)cn;

      final Certificate[] certs = https.getServerCertificates();
   ...
   *
   * @return the network port number of the URL for this HttpsConnection. The default HTTPS port
   *         number (443) is returned if there was no port number in the string passed to
   *         Connector.open.
   */
  @Override
  public int getPort() {
