...
public class Connection implements ClosedConnection {
  ...
  public javax.microedition.io.Connection open(final String name) throws IOException {
    ...
    if (portSepIndex == -1) { throw new IllegalArgumentException("Port missing"); }
    final String portToParse = name.substring(portSepIndex + 1);
    if (portToParse.length() > 0) {
      port = Integer.parseInt(portToParse);
    }
    final String host = name.substring("socket://".length(), portSepIndex);
