...
public class MRUList implements XMLItem {
  ...
  public MRUList(final Class classXMLItem, final String itemsName) {
    this.classXMLItem = classXMLItem;
    this.itemsName = itemsName;
  }

  public Object push(final Object item) {
    if (!(item instanceof XMLItem)) { throw new ClassCastException(item.getClass().getName()); }
    modified = true;
  ...
  private void fireListener(final Object item) {
    if (listener != null) {
      listener.listItemChanged(item);
    }

  }

  public void setListener(final MRUListListener l) {
  ...
  public void save(final XMLElement xml) {
    ...
    for (final Iterator iter = items.iterator(); iter.hasNext();) {
      final XMLItem element = (XMLItem)iter.next();
  ...
  public void read(final XMLElement xml) {
    ...
    maxCapacity = xml.getIntAttribute("maxCapacity", MRUList.MAXCAPACITY_DEFAULT);
    for (final Enumeration en = xml.enumerateChildren(); en.hasMoreElements();) {
      final XMLElement xmlChild = (XMLElement)en.nextElement();
      if (xmlChild.getName().equals(itemsName)) {
        try {
          final XMLItem element = (XMLItem)classXMLItem.newInstance();
    ...
    if (!items.empty()) {
      ...
      for (final ListIterator iter = items.listIterator(items.size()); iter.hasPrevious();) {
        final XMLItem element = (XMLItem)iter.previous();
