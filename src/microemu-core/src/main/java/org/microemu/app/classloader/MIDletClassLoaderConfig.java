...
public class MIDletClassLoaderConfig {
  ...
  public MIDletClassLoaderConfig() {
    ...
    delegationType = MIDletClassLoaderConfig.DELEGATION_STRICT;
  }

  public void setDelegationType(final String delegationType) throws ConfigurationException {
    if ("strict".equalsIgnoreCase(delegationType)) {
      ...
      if ((appclasses.size() != 0) || (appclasspath.size() != 0)) { throw new ConfigurationException("Can't extend system CLASSPATH"); }
  ...
  public int getDelegationType(final boolean forJad) {
  ...
  public boolean isClassLoaderDisabled() {
    return (delegationType == MIDletClassLoaderConfig.DELEGATION_SYSTEM);
  }

  public void addAppClassPath(final String path) throws ConfigurationException {
    if (delegationType == MIDletClassLoaderConfig.DELEGATION_SYSTEM) { throw new ConfigurationException("Can't extend system CLASSPATH"); }
    appclasspath.add(path);
  }

  public void addAppClass(final String className) throws ConfigurationException {
    if (delegationType == MIDletClassLoaderConfig.DELEGATION_SYSTEM) { throw new ConfigurationException("Can't extend system CLASSPATH"); }
