/**
 * MicroEmulator Copyright (C) 2001-2008 Bartek Teodorczyk <barteo@barteo.net>
 *
...
/**
 * Special InputStream wrapper for loading resources is needed to change behavior of read(byte[] b)
 * under Java SE. All existing Java ME implementations always read all bytes available in one read
 * call.
 */
public class MIDletResourceInputStream extends InputStream {
  ...
  public MIDletResourceInputStream(final InputStream is) {
    this.is = is;
  }
  ...
  public int read(final byte[] b) throws IOException {
    ...
    do {
      ...
      if (count != -1) {
        result += count;
        if (result == b.length) { return result; }
      }
