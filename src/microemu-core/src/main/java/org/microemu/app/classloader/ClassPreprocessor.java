...
public class ClassPreprocessor {
  ...
  public static byte[] instrument(final InputStream classInputStream, final InstrumentationConfig config) {
    try {
      final ClassReader cr = new ClassReader(classInputStream);
      final ClassWriter cw = new ClassWriter(0);
      final ClassVisitor cv = new ChangeCallsClassVisitor(cw, config);
      cr.accept(cv, 0);
