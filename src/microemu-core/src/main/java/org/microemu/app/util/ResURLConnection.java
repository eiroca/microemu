...
public class ResURLConnection extends URLConnection {
  ...
  protected ResURLConnection(final URL url, final Hashtable entries) {
    super(url);
  ...
  public InputStream getInputStream() throws IOException {
    ...
    final int idx = location.indexOf(ResURLConnection.PREFIX);
    if (idx == -1) { throw new IOException(); }
    location = location.substring(idx + ResURLConnection.PREFIX.length());
    final byte[] data = (byte[])entries.get(location);
