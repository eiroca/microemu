...
public final class ButtonName {
  ...
  public static final ButtonName KEY_STAR = new ButtonName(new String[] {
      "*", "STAR", "ASTERISK"
  });

  public static final ButtonName KEY_POUND = new ButtonName(new String[] {
      "#", "POUND"
  });

  private String name = "n/a";

  public static ButtonName getButtonName(final String functionName) {
    final String name = functionName.toUpperCase();
    try {
      final Field field = ButtonName.class.getField(name);
      if (field.getType() == ButtonName.class) { return (ButtonName)field.get(null); }
    }
    catch (final NoSuchFieldException e) {
    }
    catch (final IllegalAccessException e) {
    }
    ButtonName btn = (ButtonName)ButtonName.altNames.get(name);
    if (btn == null) {
      // User defined button
      btn = new ButtonName();
  ...
  static {
    ...
    final Field[] fields = ButtonName.class.getDeclaredFields();
  ...
  private ButtonName(final String name) {
    ButtonName.altNames.put(name, this);
  }

  private ButtonName(final String[] names) {
    for (final String name2 : names) {
      ButtonName.altNames.put(name2, this);
    }
  }
