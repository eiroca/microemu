...
public abstract class InputMethodImpl extends InputMethod implements Runnable {
    ...
    while (!cancel) {
      ...
      synchronized (this) {
        if (resetKey && (lastButton != null) && (inputMethodListener != null)) {
          final int caret = inputMethodListener.getCaretPosition() + 1;
          if (caret <= inputMethodListener.getText().length()) {
            lastButton = null;
            lastButtonCharIndex = -1;
            final InputMethodEvent event = new InputMethodEvent(InputMethodEvent.CARET_POSITION_CHANGED, caret, inputMethodListener.getText());
  ...
  @Override
  public void setInputMethodListener(final InputMethodListener l) {
  ...
  protected void insertText(final String str) {
    if (str.length() > 0) {
      ...
      if (!InputMethod.validate(tmp, inputMethodListener.getConstraints())) { return; }
  ...
  protected char[] filterConstraints(final char[] chars) {
    ...
    if (i != j) {
      final char[] newresult = new char[j];
  ...
  protected char[] filterInputMode(final char[] chars) {
