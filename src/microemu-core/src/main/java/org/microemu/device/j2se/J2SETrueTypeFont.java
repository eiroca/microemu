...
public class J2SETrueTypeFont implements J2SEFont {
  ...
  public J2SETrueTypeFont(final URL url, final String style, final int size, final boolean antialiasing) {
    this.url = url;
  ...
  private synchronized void checkInitialized() {
    if (!initialized) {
      ...
      try {
        final Font baseFont = Font.createFont(Font.TRUETYPE_FONT, url.openStream());
        fontMetrics = J2SETrueTypeFont.graphics.getFontMetrics(baseFont.deriveFont(awtStyle, size));
        initialized = true;
      }
