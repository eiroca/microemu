/**
 * MicroEmulator Copyright (C) 2005 Bartek Teodorczyk <barteo@barteo.net>
 *
...
public class CommandManager {
  ...
  private final CommandListener menuCommandListener = (c, d) -> {
    if (menuList == null) {
      lateInit();
    }
  ...
  private void lateInit() {
    ...
    menuList.addCommand(CommandManager.CMD_BACK);
    menuList.addCommand(CommandManager.CMD_SELECT);
  ...
  public void updateCommands(final Vector commands) {
    if (menuList == null) {
      lateInit();
    }

    final Vector buttons = DeviceFactory.getDevice().getSoftButtons();
    int numOfButtons = 0;
    ...
    final Vector commandsTable = new Vector();
    ...
    while (en.hasMoreElements()) {
      final Command commandToSort = ((CommandUI)en.nextElement()).getCommand();
    ...
    // Menu is needed
    commandsTable.insertElementAt(CommandManager.CMD_MENU, 0);
    fillPossibleCommands(buttons, commandsTable);
  ...
  private void fillPossibleCommands(final Vector buttons, final Vector commandsTable) {
    for (int i = 0; i < commandsTable.size(); i++) {
      final Enumeration en = buttons.elements();
      while (en.hasMoreElements()) {
        final SoftButton button = (SoftButton)en.nextElement();
        if ((button.getType() == SoftButton.TYPE_COMMAND)
    ...
    final Enumeration hiddenEn = buttons.elements();
    while (hiddenEn.hasMoreElements()) {
      final SoftButton hiddenButton = (SoftButton)hiddenEn.nextElement();
