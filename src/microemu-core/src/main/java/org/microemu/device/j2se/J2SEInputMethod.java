...
public class J2SEInputMethod extends InputMethodImpl {
  ...
  private class KeyReleasedDelayTask extends TimerTask {
    ...
    KeyReleasedDelayTask(final int repeatModeKeyCode) {
   ...
   *
   * @return the game action corresponding to this key, or <code>0</code> if none
   */
  @Override
  public int getGameAction(final int keyCode) {
    for (final Iterator it = DeviceFactory.getDevice().getButtons().iterator(); it.hasNext();) {
      final J2SEButton button = (J2SEButton)it.next();
      if (button.getKeyCode() == keyCode) { return ButtonDetaultDeviceKeyCodes.getGameAction(button.getFunctionalName()); }
    }
    return 0;
  }
   ...
   * @throws IllegalArgumentException if <code>gameAction</code> is not a valid game action
   */
  @Override
  public int getKeyCode(final int gameAction) {
    final ButtonName name = ButtonDetaultDeviceKeyCodes.getButtonNameByGameAction(gameAction);
    return J2SEDeviceButtonsHelper.getButton(name).getKeyCode();
  }

  /**
   * @return a string name for the key
   * @throws IllegalArgumentException if <code>keyCode</code> is not a valid key code
   */
  @Override
  public String getKeyName(final int keyCode) throws IllegalArgumentException {
    for (final Iterator it = DeviceFactory.getDevice().getButtons().iterator(); it.hasNext();) {
      final J2SEButton button = (J2SEButton)it.next();
      if (button.getKeyCode() == keyCode) { return button.getName(); }
    }
    return Character.toString((char)keyCode);
  }

  protected boolean fireInputMethodListener(final J2SEButton button, final char keyChar) {
    final MIDletAccess ma = MIDletBridge.getMIDletAccess();
    ...
    final ButtonName functionalName = button.getFunctionalName();
    ...
    if (button.isModeChange()) {
      switch (inputMethodListener.getConstraints() & TextField.CONSTRAINT_MASK) {
        ...
        case TextField.URL:
          ...
          final InputMethodEvent event = new InputMethodEvent(InputMethodEvent.CARET_POSITION_CHANGED, caret,
    ...
    if (functionalName == ButtonName.BACK_SPACE) {
      ...
      if (!InputMethod.validate(tmp, inputMethodListener.getConstraints())) { return true; }
    ...
    if (inputMethodListener.getText().length() < maxSize) {
      final StringBuffer editText = new StringBuffer(inputMethodListener.getText());
      synchronized (this) {
        lastButtonCharIndex++;
        final char[] buttonChars = filterConstraints(filterInputMode(button.getChars(getInputMode())));
        if (keyChar != '\0') {
          // Pass through letters and characters typed on keyboard but
          // not numbers that are buttons keys (presumably).
          boolean found = false;
          for (final char buttonChar : buttonChars) {
      ...
      if (!InputMethod.validate(editText.toString(), inputMethodListener.getConstraints())) { return false; }
      final InputMethodEvent event = new InputMethodEvent(InputMethodEvent.INPUT_METHOD_TEXT_CHANGED, caret, editText
          .toString());
      inputMethodListener.inputMethodTextChanged(event);
    }
    return false;
  ...
  public void buttonTyped(final J2SEButton button) {
    if (eventAlreadyConsumed) { return; }
  }
  ...
  public void buttonPressed(final J2SEButton button, final char keyChar) {
    int keyCode = keyChar;
    if ((button != null) && (keyChar == '\0')) {
      keyCode = button.getKeyCode();
    }
    ...
    final boolean rawSoftKeys = DeviceFactory.getDevice().getDeviceDisplay().isFullScreenMode();
    if ((button instanceof SoftButton) && !rawSoftKeys) {
      final Command cmd = ((SoftButton)button).getCommand();
      if (cmd != null) {
        final MIDletAccess ma = MIDletBridge.getMIDletAccess();
        if (ma == null) { return; }
        final DisplayAccess da = ma.getDisplayAccess();
  ...
  public void buttonReleased(final J2SEButton button, final char keyChar) {
    if (ignoreButtonRelease == button) { return; }
    int keyCode = keyChar;
    if ((button != null) && (keyChar == '\0')) {
      keyCode = button.getKeyCode();
    }
  ...
  public J2SEButton getButton(final KeyEvent ev) {
    J2SEButton button = J2SEDeviceButtonsHelper.getButton(ev);
    if (button != null) { return button; }
    for (final Enumeration e = DeviceFactory.getDevice().getButtons().elements(); e.hasMoreElements();) {
      button = (J2SEButton)e.nextElement();
      if (button.isChar(ev.getKeyChar(), getInputMode())) { return button; }
    }
