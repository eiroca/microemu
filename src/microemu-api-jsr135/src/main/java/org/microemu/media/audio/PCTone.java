...
public class PCTone {
  ...
  private final static AudioFormat audioFormat = new AudioFormat(sampleRate, sampleSizeInBits, channels, signed, bigEndian);
  private final static DataLine.Info dataLineInfo = new DataLine.Info(SourceDataLine.class, audioFormat);
  ...
  public void play(double frequency, double duration, double volume) {
    try {
      ...
      for (int index = 0; index < size; index++) {
        ...
        double sinValue = (Math.sin(2 * Math.PI * frequency * currentTime));

        //System.out.println("value: " + d_sampleRate * sinValue);
        //            shortBuffer.put((short)(amplify * d_sampleRate * sinValue));
      }

      PCToneRunnable pcToneRunnable = PCToneRunnablePoolFactory.getInstance(sourceDataLine, audioInputStream, audioFormat, sampleRate * MAX_TIME);
