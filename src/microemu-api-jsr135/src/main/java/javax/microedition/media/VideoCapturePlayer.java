...
public class VideoCapturePlayer implements Player {
  ...
  public synchronized void prefetch() throws MediaException {
    ...
    if (m_state > PREFETCHED) { return; }
    m_state = PREFETCHED;
  }

  private synchronized void ensureNotClosed() {
    if (m_state == CLOSED) { throw new IllegalStateException("Cannot call method on a closed player."); }
  }

  public synchronized void realize() throws MediaException {
    ensureNotClosed();
    if (m_state >= REALIZED) { return; }
    m_state = REALIZED;
  }
  ...
  public synchronized void stop() throws MediaException {
    ensureNotClosed();
    if (m_state <= REALIZED) { throw new IllegalStateException("Cannot stop an unrealized player."); }
    if (m_state == PREFETCHED) { return; }
    m_state = PREFETCHED;
    if (m_videoControl != null) {
  ...
  public Control getControl(String controlType) {
    ensureNotClosed();
    if ("VideoControl".equals(controlType) || VideoControl.class.getName().equals(controlType)) { return getVideoControl(); }
    return null;
  }
