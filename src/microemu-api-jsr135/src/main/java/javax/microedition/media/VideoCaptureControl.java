...
/**
 * Simulation video capture control that simulates capturing video within microemu
 * 
...
public class VideoCaptureControl implements VideoControl {
  ...
  public byte[] getSnapshot(String s) throws MediaException {
    ...
    doPaint(captureImage.getGraphics(), 0, 0, m_sourceWidth, m_sourceHeight);
    int[] argb = new int[m_sourceWidth * m_sourceHeight];
    // now read the RGB array
    captureImage.getRGB(argb, 0, m_sourceWidth, 0, 0, m_sourceWidth, m_sourceHeight);
    // paint it onto the BufferedImage which we need to convert to JPEG
    image.setRGB(0, 0, m_sourceWidth, m_sourceHeight, argb, 0, m_sourceWidth);
  ...
  public Object initDisplayMode(int mode, Object obj) {
    if (mode != USE_DIRECT_VIDEO) { throw new IllegalArgumentException("only USE_DIRECT_VIDEO supported for now, requested: " + mode); }
