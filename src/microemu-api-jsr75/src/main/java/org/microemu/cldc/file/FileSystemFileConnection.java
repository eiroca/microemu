...
public class FileSystemFileConnection implements FileConnection {
  ...
  FileSystemFileConnection(String fsRootConfig, String name, FileSystemConnectorImpl notifyClosed) throws IOException {
    ...
    if (hostEnd == -1) { throw new IOException("Invalid path " + name); }
    ...
    if (fullPath.length() == 0) { throw new IOException("Invalid path " + name); }
  ...
  public static File getRoot(String fsRootConfig) {
    try {
      if (fsRootConfig == null) {
        ...
        if (!fsRoot.exists()) {
          if (!fsRoot.mkdirs()) { throw new RuntimeException("Can't create filesystem root " + fsRoot.getAbsolutePath()); }
        ...
        if (!fsRoot.isDirectory()) { throw new RuntimeException("Can't find filesystem root " + fsRoot.getAbsolutePath()); }
        return fsRoot;
      }
    }
  ...
  public void create() throws IOException {
    ...
    doPrivilegedIO(new PrivilegedExceptionAction() {
      ...
      public Object run() throws IOException {
        if (!file.createNewFile()) { throw new IOException("File already exists  " + file.getAbsolutePath()); }
        return null;
      }
    });
  }
  ...
  public String getPath() {
    ...
    // /<root>/<directory>/
    if (isRoot) { return DIR_SEP + fullPath + DIR_SEP; }

    int pathEnd = fullPath.lastIndexOf(DIR_SEP);
    if (pathEnd == -1) { return DIR_SEP_STR; }
  ...
  public void mkdir() throws IOException {
    ...
    doPrivilegedIO(new PrivilegedExceptionAction() {
      ...
      public Object run() throws IOException {
        if (!file.mkdir()) { throw new IOException("Can't create directory " + file.getAbsolutePath()); }
        return null;
      }
    });
  }
  ...
  private Enumeration listPrivileged(final String filter, boolean includeHidden) throws IOException {
    if (!this.file.isDirectory()) { throw new IOException("Not a directory " + this.file.getAbsolutePath()); }
  ...
  private void throwOpenDirectory() throws IOException {
    if (this.isDirectory) { throw new IOException("Unable to open Stream on directory"); }
  }
  ...
  public InputStream openInputStream() throws IOException {
    ...
    if (this.opendInputStream != null) { throw new IOException("InputStream already opened"); }
    /**
     * Trying to open more than one InputStream or more than one OutputStream from a
     * StreamConnection causes an IOException.
     */
  ...
  private OutputStream openOutputStream(final boolean append) throws IOException {
    ...
    if (this.opendOutputStream != null) { throw new IOException("OutputStream already opened"); }
    /**
     * Trying to open more than one InputStream or more than one OutputStream from a
     * StreamConnection causes an IOException.
     */
  ...
  public void rename(final String newName) throws IOException {
    throwClosed();
    if (newName.indexOf(DIR_SEP) != -1) { throw new IllegalArgumentException("Name contains path specification " + newName); }
    doPrivilegedIO(new PrivilegedExceptionAction() {

      public Object run() throws IOException {
        File newFile = new File(file.getParentFile(), newName);
        if (!file.renameTo(newFile)) { throw new IOException("Unable to rename " + file.getAbsolutePath() + " to "
  ...
  private void fileSetJava16(String mehtodName, final Boolean param) throws IOException {
    if (java15) { throw new IOException("Not supported on Java version < 6"); }
    // Use Java6 function in reflection.
    try {
      final Method setWritable = file.getClass().getMethod(mehtodName, new Class[] {
          boolean.class
      });
      doPrivilegedIO(new PrivilegedExceptionAction() {
        ...
        public Object run() throws IOException {
          try {
            setWritable.invoke(file, new Object[] {
                param
            });
          }
          catch (Exception e) {
  ...
  private long getFileValueJava6(String mehtodName) throws SecurityException {
    if (java15) { throw new SecurityException("Not supported on Java version < 6"); }
    // Use Java6 function in reflection.
    try {
