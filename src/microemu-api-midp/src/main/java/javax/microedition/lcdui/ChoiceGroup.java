...
public class ChoiceGroup extends Item implements Choice {
  ...
  ChoiceGroup(String label, int choiceType, boolean validateChoiceType) {
    ...
    if (validateChoiceType) {
      if (choiceType != Choice.POPUP && choiceType != Choice.MULTIPLE && choiceType != Choice.EXCLUSIVE) { throw new IllegalArgumentException("Illegal choice type"); }
    }
  ...
  public void delete(int itemNum) {
    if (ui.getClass().getName().equals("org.microemu.android.device.ui.AndroidChoiceGroupUI")) {
      ...
      if (itemNum < 0 || itemNum >= numOfItems) { throw new IndexOutOfBoundsException(); }
  ...
  public Image getImage(int elementNum) {
    if (elementNum < 0 || elementNum >= numOfItems) { throw new IndexOutOfBoundsException(); }
   ...
   * selectedArray_return. NOTE: this is a result parameter. It must be at least as long as the size
   * of the ChoiceGroup as returned by size(). If the array is longer, the extra elements are set to
   * false.
   * 
   * For ChoiceGroup objects of type MULTIPLE, any number of elements may be selected and set to
   * true in the result array. For ChoiceGroup objects of type EXCLUSIVE, exactly one element will
   * be selected, unless there are zero elements in the ChoiceGroup.
   */
  public int getSelectedFlags(boolean[] selectedArray) {
    if (ui.getClass().getName().equals("org.microemu.android.device.ui.AndroidChoiceGroupUI")) {
      ...
      if (selectedArray == null) { throw new NullPointerException(); }
      if (selectedArray.length < numOfItems) { throw new IllegalArgumentException(); }
  ...
  /**
   * Returns the index number of an element in the ChoiceGroup that is selected. For ChoiceGroup
   * objects of type EXCLUSIVE there is at most one element selected, so this method is useful for
   * determining the user's choice. Returns -1 if there are no elements in the ChoiceGroup.
   * 
   * For ChoiceGroup objects of type MULTIPLE, this always returns -1 because no single value can in
   * general represent the state of such a ChoiceGroup. To get the complete state of a MULTIPLE
  ...
  public String getString(int elementNum) {
    if (ui.getClass().getName().equals("org.microemu.android.device.ui.AndroidChoiceGroupUI")) {
      if (elementNum < 0 || elementNum >= ((ChoiceGroupUI)ui).size()) { throw new IndexOutOfBoundsException(); }
  ...
  public void setSelectedFlags(boolean[] selectedArray) {
    if (ui.getClass().getName().equals("org.microemu.android.device.ui.AndroidChoiceGroupUI")) {
      ...
      if (selectedArray == null) { throw new NullPointerException(); }
      if (selectedArray.length < numOfItems) { throw new NullPointerException(); }
  ...
  class ChoiceItem extends ImageStringItem {
    ...
    int paint(Graphics g) {
      if (stringComponent == null) { return 0; }
      int widthAddition = 0;
      if (box != null) {
    ...
    void setFont(Font f) {
      ...
      if (f.getHeight() == font.getHeight()) font = f;
    }

    void setSelectedState(boolean state) {
      selected = state;
      if (choiceType != Choice.IMPLICIT && choiceType != Choice.POPUP) {
        box = (Choice.EXCLUSIVE == choiceType ? (state ? imgRadioOn : imgRadioOff) : (state ? imgMultiOn : imgMultiOff));
      }
    }
  }
