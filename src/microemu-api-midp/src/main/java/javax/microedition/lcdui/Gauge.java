...
public class Gauge extends Item {
  ...
  // these are for render of indefinite running and updating gauges
  ...
  void updateIndefiniteFrame() {
    if (hasIndefiniteRange() && ((getValue() == Gauge.CONTINUOUS_RUNNING) || (getValue() == Gauge.INCREMENTAL_UPDATING))) {
      if (indefiniteFrame + 1 < Gauge.IDEFINITE_FRAMES) indefiniteFrame++;
      else indefiniteFrame = 0;
      repaint();
    }
  }
  // ITEM methods
  ...
  int paint(Graphics g) {
    ...
    if (hasIndefiniteRange()) {
      if (getValue() == Gauge.CONTINUOUS_IDLE || getValue() == Gauge.INCREMENTAL_IDLE) {
  ...
  public void setPreferredSize(int w, int h) {
    ...
    if (owner != null && owner instanceof Alert) return;
    else super.setPreferredSize(w, h);
  }
  ...
  public void setDefaultCommand(Command cmd) {
    if (owner != null && owner instanceof Alert) return;
    else super.setDefaultCommand(cmd);
  }

  public void setItemCommandListener(ItemCommandListener l) {
    if (owner != null && owner instanceof Alert) return;
    else super.setItemCommandListener(l);
  }
