...
public class Alert extends Screen {
  ...
  public void setTimeout(int time) {
    if (time != FOREVER && time <= 0) { throw new IllegalArgumentException(); }
    // XXX stop timeout thread!
    if (time != FOREVER && getCommands().size() > 1) time = FOREVER;
    this.time = time;
  }
  ...
  int traverse(int gameKeyCode, int top, int bottom) {
    ...
    if (gameKeyCode == 1 && top != 0) { return -f.getHeight(); }
    if (gameKeyCode == 6 && bottom < getContentHeight()) { return f.getHeight(); }
    return 0;
  }
