...
public class Form extends Screen {
  ...
  /**
   * Verify that the item is non null and is not owned by this form or anyone else. If all is ok set
   * the owner to this Form
   * 
  ...
  private void verifyItem(Item item) {
    ...
    if (item.getOwner() != null) { throw new IllegalStateException("item is already owned"); }
  ...
  private void verifyItemNum(int itemNum) {
    if (itemNum < 0 || itemNum >= numOfItems) { throw new IndexOutOfBoundsException("item number is outside range of Form"); }
  }

  Vector getCommands() {
    Vector formCommands = super.getCommands();
    if (focusItemIndex < 0) return formCommands;
    Item item = items[focusItemIndex];
    Vector itemCommands = item.commands;
    if (itemCommands.isEmpty()) return formCommands;
    ...
    for (int i = 0; i < formCommands.size(); i++) allCommands.add(formCommands.elementAt(i));
