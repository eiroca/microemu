/**
 * MicroEmulator Copyright (C) 2005 Andres Navarro
 *
...
public class Sprite extends Layer {
  ...
  public Sprite(Image img, int frameWidth, int frameHeight) {
    ...
    if (img.getWidth() % frameWidth != 0 || img.getHeight() % frameHeight != 0) throw new IllegalArgumentException();
    this.img = img;
  ...
  public final boolean collidesWith(Image image, int iX, int iY, boolean pixelLevel) {
    ...
    if (pixelLevel) return collidesWithPixelLevel(image, iX, iY);
    else return collidesWith(image, iX, iY);
  }
  ...
  public final boolean collidesWith(TiledLayer layer, boolean pixelLevel) {
    ...
    // second and third parameters are dont care
    if (pixelLevel) return collidesWithPixelLevel(layer, 0, 0);
    else return collidesWith(layer, 0, 0);
  }

  public final boolean collidesWith(Sprite otherSprite, boolean pixelLevel) {
    if (otherSprite == null) { throw new NullPointerException(); }
    int sX, sY;
    int sW, sH;
    // only check collision if both are visible
    if (!otherSprite.isVisible() || !this.isVisible()) return false;
    // second and third parameters are dont care
    if (pixelLevel) return collidesWithPixelLevel(otherSprite, 0, 0);
    else return collidesWith(otherSprite, 0, 0);
  }
  ...
  public void setRefPixelPosition(int x, int y) {
    ...
    switch (transform) {
      ...
      case TRANS_MIRROR_ROT90:
        ...
        break;
      default:
        // cannot really happen, but the return keeps the compiler happy (otherwise it'll report variable may not be initialized)
        return;
    }
  ...
  public void setFrameSequence(int[] sequence) {
    ...
    for (int i = 0; i < l; i++) {
      ...
      if (value > max || value < 0) throw new ArrayIndexOutOfBoundsException();
    }
  ...
  public void setFrame(int frame) {
    ...
    if (frame < 0 || frame >= l) { throw new IndexOutOfBoundsException(); }
    this.frame = frame;
  }

  public void nextFrame() {
    if (frame == ((sequence == null) ? rows * cols : sequence.length) - 1) frame = 0;
    else frame++;
  }

  public void prevFrame() {
    if (frame == 0) frame = ((sequence == null) ? rows * cols : sequence.length) - 1;
    else frame--;
  }
        ...
        if (transform != TRANS_NONE) {
          ...
          switch (transform) {
            ...
            case TRANS_MIRROR_ROT90:
              ...
              break;
            default:
              // cant really happen, but the return keeps the compiler happy (otherwise it'll report variable may not be initialized)
              return;
          }
  ...
  public void setTransform(int transform) {
    ...
    // calculate the coordinates of refPixel in the new transform relative to x, y
    int newRefX, newRefY;
    switch (transform) {
      case TRANS_NONE:
        newRefX = refX;
    ...
    // calculate the coordinates of refPixel in the current transform relative to x, y
    int curRefX, curRefY;
   ...
   * of the code
   *
   * For both methods, the second and third parameters are significant only if o is an Image,
  ...
  private synchronized boolean collidesWith(Object o, int oX, int oY) {
    ...
    while (another) {
      ...
      int cW = t.collWidth;
      int cH = t.collHeight;
      // if there is a zero in a dimension then it cannot intersect with anything!
    ...
    if (o instanceof TiledLayer) {
      ...
      // travel across all cells in the collision rectangle
          ...
          for (int row = minR; row <= maxR; row++) {
            for (int col = minC; col <= maxC; col++) {
              ...
              if (cell == 0) {
                ...
                img.getRGB(rgbDataAux, c, rW, xSrc + minX, ySrc + minY, maxX - minX + 1, maxY - minY + 1);
              }
            }
          }
        }
