 ...
 * limitations.
 * 
 * Contributor(s): 3GLab Robert Helmer
 */
...
public class TextField extends Item {
  ...
  public TextField(String label, String text, int maxSize, int constraints) {
    ...
    if (!InputMethod.validate(text, constraints)) { throw new IllegalArgumentException(); }
    if (maxSize <= 0) { throw new IllegalArgumentException(); }
  ...
  public String getString() {
    if (ui.getClass().getName().equals("org.microemu.android.device.ui.AndroidTextFieldUI")) { return ((TextFieldUI)ui).getString(); }
  ...
  void setString(String text, int caret) {
    if (!InputMethod.validate(text, constraints)) { throw new IllegalArgumentException("text  is illegal for the current input constraints"); }
    if (text == null) {
      field = "";
      stringComponent.setText("");
    }
    else {
      if (text.length() > maxSize) { throw new IllegalArgumentException("text exceeds the current maximum capacity"); }
      field = text;
  ...
  public int getChars(char[] data) {
    if (data.length < field.length()) { throw new ArrayIndexOutOfBoundsException(); }
  ...
  public void setChars(char[] data, int offset, int length) {
    if (data == null) {
      ...
      if (!InputMethod.validate(newtext, constraints)) { throw new IllegalArgumentException(); }
      setString(newtext);
    }
    repaint();
  }
  ...
  public void insert(String src, int position) {
    if (!InputMethod.validate(src, constraints)) { throw new IllegalArgumentException(); }
    if (field.length() + src.length() > maxSize) { throw new IllegalArgumentException(); }
    String newtext = "";
    if (position > 0) {
      if (ui.getClass().getName().equals("org.microemu.android.device.ui.AndroidTextFieldUI")) {
  ...
  public void insert(char[] data, int offset, int length, int position) {
    if (offset + length > data.length) { throw new ArrayIndexOutOfBoundsException(); }
    insert(new String(data, offset, length), position);
  }

  public void delete(int offset, int length) {
    if (offset + length > field.length()) { throw new StringIndexOutOfBoundsException(); }
    String newtext = "";
    if (offset > 0) {
  ...
  public void setConstraints(int constraints) {
    if ((constraints & TextField.CONSTRAINT_MASK) < ANY
        || (constraints & TextField.CONSTRAINT_MASK) > DECIMAL) { throw new IllegalArgumentException("constraints " + constraints + " is an illegal value"); }
