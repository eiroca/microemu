...
public class Display {
  ...
  /**
   * Wrap a key event as a runnable so it can be thrown into the event processing queue. Note that
   * this may be a bit buggy, since events are supposed to propogate to the head of the queue and
   * not get tied behind other repaints or serial calls in the queue.
   * 
   * @author radoshi
   * 
   */
  ...
  private class DisplayAccessor implements DisplayAccess {
    ...
    private void processGameCanvasKeyEvent(GameCanvas c, int k, boolean press) {
      ...
      if (gameCode != 0) {
        ...
        else access.recordKeyReleased(c, gameCode);
  ...
  /**
   * Wrap any runnable as a timertask so that when the timer gets fired, the runnable gets run
   * 
   * @author radoshi
   * 
   */
  ...
  public void setCurrent(final Displayable nextDisplayable) {
    if (nextDisplayable == current) { return; }
  ...
  public void setCurrent(Alert alert, Displayable nextDisplayable) {
    if (alert == null) { throw new NullPointerException("alert"); }
    if (nextDisplayable == null) { throw new NullPointerException("nextDisplayable"); }
    if (nextDisplayable instanceof Alert) { throw new IllegalArgumentException("nextDisplayable"); }
