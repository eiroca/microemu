...
public class RecordStoreImpl extends RecordStore {
  ...
  public RecordStoreImpl(RecordStoreManager recordStoreManager) throws IOException {
    this.recordStoreManager = recordStoreManager;
  }

  public int readHeader(DataInputStream dis) throws IOException {
    for (int i = 0; i < fileIdentifier.length; i++) {
      if (dis.read() != fileIdentifier[i]) { throw new IOException(); }
    }
  ...
  public void closeRecordStore() throws RecordStoreNotOpenException, RecordStoreException {
    if (!open) { throw new RecordStoreNotOpenException(); }
  ...
  public int addRecord(byte[] data, int offset, int numBytes) throws RecordStoreNotOpenException, RecordStoreException, RecordStoreFullException {
    if (!open) { throw new RecordStoreNotOpenException(); }
    if (data == null && numBytes > 0) { throw new NullPointerException(); }
    if (numBytes > recordStoreManager.getSizeAvailable(this)) { throw new RecordStoreFullException(); }
    // lastRecordId needs to hold correct number, all records have to be preloaded
    enumerateRecords(null, null, false);
  ...
  public int getRecordSize(int recordId) throws RecordStoreNotOpenException, InvalidRecordIDException, RecordStoreException {
    ...
    synchronized (this) {
      ...
      if (data == null) {
        ...
        if (data == null) { throw new InvalidRecordIDException(); }
      }
      return data.length;
    }
  }
