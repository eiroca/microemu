/**
 * Siemens API for MicroEmulator Copyright (C) 2003 Markus Heberling <markus@heberling.net>
 *
...
public class Sprite extends Layer {
  ...
  public Sprite(Image image, int frameWidth, int frameHeight) {
    ...
    if ((frameWidth < 1 || frameHeight < 1) ||
        ...
        ((image.getHeight() % frameHeight) != 0)) { throw new IllegalArgumentException(); }
  ...
  public void setFrame(int sequenceIndex) {
    if (sequenceIndex < 0 || sequenceIndex >= frameSequence.length) { throw new IndexOutOfBoundsException(); }
  ...
  public final void paint(Graphics g) {
    ...
    if (visible) {
      ...
      g.drawImage(sourceImage, this.x - frameCoordsX[frameSequence[sequenceIndex]], this.y - frameCoordsY[frameSequence[sequenceIndex]], Graphics.TOP | Graphics.LEFT);
  ...
  public void setFrameSequence(int sequence[]) {
    ...
    for (int i = 0; i < sequence.length; i++) {
      if (sequence[i] < 0 || sequence[i] >= numberFrames) { throw new ArrayIndexOutOfBoundsException(); }
    }
  ...
  public void setImage(Image img, int frameWidth, int frameHeight) {
    ...
    if ((frameWidth < 1 || frameHeight < 1) || ((img.getWidth() % frameWidth) != 0) || ((img.getHeight() % frameHeight) != 0)) { throw new IllegalArgumentException(); }
  ...
  public final boolean collidesWith(Sprite s, boolean pixelLevel) {
    ...
    if (intersectRect(otherLeft, otherTop, otherRight, otherBottom, left, top, right, bottom)) { return true; }
    return false;
  }
  ...
  public final boolean collidesWith(TiledLayer t, boolean pixelLevel) {
    ...
    for (int row = startRow; row <= endRow; row++) {
      for (int col = startCol; col <= endCol; col++) {
        if (t.cellMatrix[row][col] != 0) { return true; }
      }
    }
